...
export interface Handlers<S> extends KisaHandlers<S> {
  login: KisaHandler<S, ReqTypes.Login>; 
  listPosts: KisaHandler<S, ReqTypes.ListPosts>; 
  createPost: KisaHandler<S, ReqTypes.CreatePost>; 
  publishPost: KisaHandler<S, ReqTypes.PublishPost>; 
  listMyPosts: KisaHandler<S, ReqTypes.ListMyPosts>; 
  deletePost: KisaHandler<S, ReqTypes.DeletePost>; 
}

export interface Middlewares<S> extends KisaMiddlewares<S> {
  ratelimit: KisaMiddleware<S>; 
}

export interface SecurityHandlers<S> extends KisaSecurityHandlers<S> {
  jwt: (config: string[]) => KisaMiddleware<S>; 
}
